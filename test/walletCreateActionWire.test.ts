...
function fixture() {
  ...
  const authorize = vi
    .spyOn(manager, "ensureSpendingAuthorization")
  ...
  outputs: [
    {
      lockingScript: "51",
      satoshis: 1000,
      outputDescription: "Application output",
    },
  ],
};
...
describe("published wallet createAction binary compatibility", () => {
  it.each([false, true])(
    "returns an authorized action with partial=%s",
    async (partial) => {
      ...
      if (partial) {
        ...
        expect(Array.from(result.signableTransaction!.tx)).toEqual(
          tx.toAtomicBEEF(),
        );
      } else {
